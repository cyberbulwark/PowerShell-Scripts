...
.csvlink { display: inline-block; margin-left: 8px; padding: 8px 12px; border-radius: 4px; background: #217346; color: white; text-decoration: none; font-weight: 600; }
.table-container { width: 100%; max-width: 100%; overflow-x: auto; margin-bottom: 24px; border: 1px solid #ccc; }
...
th, td { border-right: 1px solid #ccc; border-bottom: 1px solid #ccc; padding: 6px; vertical-align: top; min-width: 110px; max-width: 320px; overflow-wrap: anywhere; }
...
$csvLinkHtml = if ($selectedFormats -contains 'CSV') {
    '<a class="csvlink" href="SQL-Server-Inventory.csv" download>Download CSV</a>'
}
else { '' }
$body = @"
...
<div class="table-container">$databaseHtml</div>
<h2>Scanned Computers Without SQL Server</h2>
<div class="table-container">$noSqlHtml</div>
<p class="small">CSV files for SQL instances, databases, unreachable computers, and scanned computers without SQL Server are saved beside this report.</p>
<script>
...
document.addEventListener('keydown', function(event) {
    if (event.key === '/' && document.activeElement !== document.getElementById('inventorySearch')) {
        event.preventDefault();
        document.getElementById('inventorySearch').focus();
    }
    if (event.key === 'Escape') { clearInventorySearch(); }
});
</script>
"@
